...
public struct SpendingRecord has copy, drop, store {
    amount: u64,
...
#[error]
const EDUCATION_LEVEL_MISMATCH: vector<u8> =
    b"Student education level doesn't match grant requirements";
#[error]
...
public fun apply_for_grant(
    ...
    assert!(
        student_profile.education_level == grant.target_education_level,
...
public fun approve_grant_application(
    ...
    // ←――――――――――――――――――
    // **Re-share** the mutated StudentWallet so the test harness can retrieve it
    //transfer::share_object(student_wallet);
}
...
public fun deactivate_grant(_: &AdminCap, grant: &mut Grant) {
